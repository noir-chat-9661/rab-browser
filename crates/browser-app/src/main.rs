...
/// Builds the Windows menu bar, but doesn't wire up its command handler or
/// attach it to a window yet (see `install_app_menu_windows_handler` and
/// `attach_app_menu_windows`) — the accelerator table this returns needs to
/// exist before the event loop is built (so its `EventLoopProxy` isn't
/// available yet either), while attaching needs a window HWND that doesn't
/// exist until after.
#[cfg(target_os = "windows")]
fn build_app_menu_windows() -> Menu {
    let quit = PredefinedMenuItem::quit(None);
    let file_menu =
    ...
    Menu::with_items(&[&file_menu, &edit_menu, &help_menu]).expect("failed to build the menu bar")
}

/// Wires up the Windows menu's command handler. Split out from
/// `build_app_menu_windows` because it needs an `EventLoopProxy`, which only
/// exists once the event loop this menu's accelerator table was registered
/// with has actually been built.
#[cfg(target_os = "windows")]
fn install_app_menu_windows_handler(
    commands_tx: Sender<String>,
    event_loop_proxy: EventLoopProxy<McpRequest>,
) {
    // IDs are set explicitly in build_app_menu_windows (not autogenerated),
    // so they can be matched here by string without threading MenuId values
    // between the two functions.
    MenuEvent::set_event_handler(Some(move |event: MenuEvent| {
        let command = match event.id.0.as_str() {
            "rab-browser.mcp-help" => Some(r#"{"type":"open_mcp_help"}"#),
            "rab-browser.settings" => Some(r#"{"type":"open_settings"}"#),
            _ => None,
        };
        if let Some(command) = command
            && commands_tx.send(command.to_owned()).is_ok()
...
/// Attaches a menu built by `build_app_menu_windows` to a window's menu bar.
/// Accelerators (Ctrl+C etc.) only work if the win32 message loop also calls
/// `TranslateAcceleratorW` with this menu's `haccel()` — see the
/// `with_msg_hook` registration in `main` — this call alone only makes the
/// menu bar visible and clickable.
#[cfg(target_os = "windows")]
fn attach_app_menu_windows(menu: &Menu, hwnd: isize) {
...
/// instance already holds it (or something else does — the control-socket
/// handshake, not the bind, is what actually confirms it's rab-browser).
...
fn relay_stdio_to_existing_instance() -> bool {
    ...
    // would hang the whole relay process forever in that case; letting the
    // process exit once the socket->stdout copy below finishes reclaims it.
...
fn main() -> wry::Result<()> {
    ...
    // time. If one is already running (it holds the control port and its
    // auth token checks out), an `--mcp` launch attaches to it over that
    // socket instead of opening a second window and exits once done. If the
    // port is held but the handshake fails (some other process has it), or
    // this is a plain launch, fall through to opening a window here as
    // usual (raising an existing window across processes is out of scope).
    let control_listener = try_bind_control_socket();
    if control_listener.is_none() && mcp_enabled && relay_stdio_to_existing_instance() {
        return Ok(());
    }

    let (commands_tx, commands_rx) = mpsc::channel::<String>();

    let mut event_loop_builder = EventLoopBuilder::<McpRequest>::with_user_event();
    // Building the Windows menu (and grabbing its accelerator table) has to
    // happen before the event loop is built — `with_msg_hook` is only on
    // the builder — even though attaching the menu to a window has to wait
    // until the window (and its HWND) exists, further down.
    #[cfg(target_os = "windows")]
    let windows_menu = {
        use tao::platform::windows::EventLoopBuilderExtWindows;
        use windows::Win32::UI::WindowsAndMessaging::{HACCEL, MSG, TranslateAcceleratorW};

        let menu = build_app_menu_windows();
        // Menu::haccel() returns the accelerator table as a raw handle
        // value; wrap it once here so the message hook itself is a plain
        // FFI call.
        let haccel = HACCEL(menu.haccel() as *mut std::ffi::c_void);
        event_loop_builder.with_msg_hook(move |msg| {
            let msg = msg as *const MSG;
            // SAFETY: tao guarantees `msg` points to a valid MSG for the
            // duration of this callback; `haccel` outlives the event loop
            // (the menu it comes from is kept alive via `_keep_app_menu_alive`).
            unsafe { TranslateAcceleratorW((*msg).hwnd, haccel, msg) != 0 }
        });
        menu
    };
    ...
    let app_menu = {
        ...
        install_app_menu_windows_handler(commands_tx.clone(), event_loop.create_proxy());
        attach_app_menu_windows(&windows_menu, window.hwnd());
        windows_menu
    };
    #[cfg(target_os = "macos")]
